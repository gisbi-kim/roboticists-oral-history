...
시각화에서는 비교 기준점이 되는 두 항목을 원본 타입에서 분리해 별도 표시 타입으로 보여준다.

- **출생** — 원본 `개인사 / birth`에서 파생
- **연구 시작** — 원본 `교육 / ms_enroll·phd_enroll`에서 우선 파생하고, 없으면 대학원 단계의 `ms_graduate·phd_graduate·phd_thesis·proj_start` 중 가장 이른 사건으로 보완

이 두 기준점은 어떤 타입 필터를 켜도 항상 표시된다. 따라서 `개인사`는 출생 이후의 개인 이동·가족·사망 등으로, `교육`은 학위 과정·전공·졸업·지도교수 등으로 해석하면 된다.
